...
mean = Dataset(path + "anal00061")
t2mean = wrf.getvar(mean, "T2")
...
#    print(files[f])
#    t2 = wrf.to_np(wrf.getvar(ncfile, "T2", meta = False))
##    diff = t2 - t2mean
#
#    ax = fig.add_subplot(10, 6, f+1)
...
#    cf = m.contourf(x, y, t2, levels = levels, cmap = 'RdYlBu_r')
#    cbar = m.colorbar(cf, ticks = ticks, drawedges = True, location='bottom')
#    cbar.ax.tick_params(labelsize = 3)
#    m.contour(x, y, t2, levels = levels, colors = '#f4fbd2', linewidths = 0.1)
#    m.drawcoastlines(linewidth = 0.5)
#    m.drawcountries(linewidth = 0.5)
#    #plt.tight_layout(pad = 0.9, h_pad = 0.5, w_pad = 0.1)
#
#
#plt.subplots_adjust(left  = 0.01, right = 0.99, bottom = 0.01, top = 0.99, wspace = 0.00001)    
##plt.tight_layout()
#fig.savefig("t2_wrfout.png")
#plt.close()
##############################################################
...
for f in range(len(files)):
    ...
    m = Basemap(resolution = 'i', llcrnrlon = llon, llcrnrlat = llat, urcrnrlon = rlon, urcrnrlat = rlat,
                ...
                ax = grid[f])
    x, y = m(wrf.to_np(lon), wrf.to_np(lat))
    cf = grid[f].contourf(x, y, t2, levels = levels, cmap = 'RdYlBu_r')
    grid[f].contour(x, y, t2, levels = levels, colors = '#f4fbd2', linewidths = 0.1)
    m.drawcoastlines(linewidth = 0.5)
    m.drawcountries(linewidth = 0.5)
